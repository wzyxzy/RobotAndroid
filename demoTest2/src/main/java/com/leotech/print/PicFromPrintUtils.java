...
import com.leotech.print.SerialPrinter.PrinterTask;
...
public class PicFromPrintUtils {
     ...
     * @return
     */
    public static byte[] draw2PxPoint360(PrinterTask task,Bitmap bit) {    
        ...
        for (int j = 0; j < 15; j++) {
            ...
            try {
            //LeoRobot.print(data);
            //LeoRobot.print("diyihang".getBytes("GBK"));
			task.send(data);
			task.send("diyihang".getBytes("GBK"));
            Thread.sleep(300);
        } catch (Exception e) {
			e.printStackTrace();
		}
            String content = HexDump.toHexString(data);
            Log.v("main","  print  line="+j+" content ="+content);
            k = 0;
        }
        return data;
    }
    /*************************************************************************
     * 我们的热敏打印机是RP-POS80S或RP-POS80P或RP-POS80CS或RP-POS80CP打印机
     * 240*240的图片，8个字节（8个像素点）是一个二进制，将二进制转化为十进制数值
     * y轴：24个像素点为一组，即240就是10组（0-9）
     * x轴：240个像素点（0-239）
     * 里面的每一组（24*240），每8个像素点为一个二进制，（每组有3个，3*8=24）
     **************************************************************************/
    /**
     * 把一张Bitmap图片转化为打印机可以打印的bit(将图片压缩为360*360)
     * 效率很高（相对于下面）
     ...
     * @return
     */
    public static byte[] draw2PxPoint(PrinterTask task,Bitmap bit) {    
        //byte[] data = new byte[7290];
        ...
        for (int j = 0; j < 10; j++) {
            ...
            try {
	           // LeoRobot.print(data);
	            ...
	            if(j<3)
				{
					if(SocketThread.baudrate.equals("9600"))
						Thread.sleep(1200);
					else
						Thread.sleep(300);
				}
